...
def aes_encryption_algo(key_matrix, state_matrix):
    ...
    key_schedule_time = (key_schedule_end_time - key_schedule_start_time) * 1000
    ...
    return state_matrix, key_schedule_time
...
def cbc_encryption(plaintext_bv, key_matrix, iv_matrix):
    ...
    for i in range(0, plaintext_bv.length(), 128):
        ...
        # AES encryption
        encrypted_matrix, key_schedule_time = aes_encryption_algo(key_matrix, state_matrix)
    ...
    return full_ciphertext_bv, key_schedule_time
...
def send_receive(input_plaintext=None, input_key=None, role=None):
    ...
    if role == "Alice":
        ...
        full_ciphertext_bv, key_schedule_time = cbc_encryption(plaintext_bv, key_matrix, iv_matrix)
...
def print_hex_byte_by_byte(hex_bv, padding):
    for i in range(0, hex_bv.length(), 8):
        byte = hex_bv[i:i + 8]
        if i == 0:
            if padding == 1:
                print("In Hex:",byte.get_bitvector_in_hex().upper(), end=' ')
            else:
                print("In Hex (After Padding):", byte.get_bitvector_in_hex().upper(), end=' ')
        else:
...
def input_process():
    ...
    print_hex_byte_by_byte(key_bv, 0)
    print("\n")

    input_plaintext = input("Plain Text: ")
    plaintext_bv = BitVector(textstring=input_plaintext)
    print("In ASCII: ", input_plaintext)
    print_hex_byte_by_byte(plaintext_bv, 0)
    print()
    ...
    print_hex_byte_by_byte(plaintext_bv, 1)
    print()

    encryp_start_time = time.time()
    full_ciphertext_bv, key_schedule_time = cbc_encryption(plaintext_bv, key_matrix, iv_matrix)
    encryp_end_time = time.time()
    print("\n")
    print("Ciphered Text:")
    print_hex_byte_by_byte(full_ciphertext_bv, 0)
    print()
    ...
    print_hex_byte_by_byte(decrypted_bv, 0)
    print()
    ...
    print("Encryption time: ", (encryp_end_time - encryp_start_time)*1000, "ms")
    print("Decryption time: ", (decrypt_end_time - decrypt_start_time)*1000, "ms")
